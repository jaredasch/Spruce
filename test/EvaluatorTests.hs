...
evalTestSuite =
  TestList
    [ "test/queries/foreach.bql" $=$ int 55,
      ...
      "test/queries/fib.bql" $=$ returnVal (ArrayVal (IntVal <$> [10946, 17711, 28657]) `as` ArrayT IntT),
      "test/queries/memoized_fib.bql" $=$ returnVal (ArrayVal (IntVal <$> [10946, 17711, 28657]) `as` ArrayT IntT),
